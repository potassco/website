...
async function init() {
  ...
  await micropip.install(
    new URL(
      "./clingo-6.0.0-cp314-cp314-pyemscripten_2026_0_wasm32.whl",
      self.location.href,
    ).toString(),
  );
  pyodide.setStdout({
    batched: (msg) => postMessage({ type: "stdout", value: msg }),
  });
  pyodide.setStderr({
    batched: (msg) => postMessage({ type: "stderr", value: msg }),
  });
